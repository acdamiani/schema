...
public class ConeCheck : Decorator
{
        ...
        else
        {
            ...
            foreach (Collider c in colliders)
            {
                if (c.transform != agent.transform && PointInsideCone(
                    agent.transform.position + rotatedOffset, rotation * Vector3.forward, rayRange, radius, c.transform.position
                    ) && tagFilter.tags.Contains(c.transform.tag))
                {
                    ...
                    (c.transform.position - (agent.transform.position + rotatedOffset)).normalized, out hit, rayRange, -1, QueryTriggerInteraction.UseGlobal))
                    {
                        if (hit.collider == c)
    ...
    private Dictionary<Ray, RaycastHit> GetHitInfo(Ray[] rays)
    {
        ...
        for (int i = 0; i < rays.Length; i++)
        {
            ...
            if (Physics.Raycast(rays[i], out hitInfo, rayRange))
            {
    ...
    private RayRepresentation[] GenerateRays(SchemaAgent agent)
    {
        ...
        for (int i = 0; i < rays.Length; i++)
        {
            ...
            Vector3 direction = ((modified + rotatedOffset) - (agent.transform.position + rotatedOffset)) * rayRange;
