...
namespace Schema.Builtin.Conditionals
{
    ...
    {
        public BlackboardEntrySelector<float> rayRange = new BlackboardEntrySelector<float>(10.0f);
        [Range(0f, 85f)] public float halfAngle = 15.0f;
        public float coneDirection = 0;
        [Tooltip("How many rays to send to check for colliding Game Objects. Increasing this value will reduce performance but will result in more accurate results for smaller objects.")]
        [Range(10, 100)]
            ...
            else
            {
                ...
                foreach (Collider c in colliders)
                {
                    if (c.transform != agent.transform && PointInsideCone(
                        agent.transform.position + rotatedOffset, rotation * Vector3.forward, rayRange.value, radius, c.transform.position
                        ) && tagFilter.tags.Contains(c.transform.tag))
                    {
                        ...
                        (c.transform.position - (agent.transform.position + rotatedOffset)).normalized, out hit, rayRange.value, -1, QueryTriggerInteraction.UseGlobal))
                        {
                            if (hit.collider == c)
        ...
        private Dictionary<Ray, RaycastHit> GetHitInfo(Ray[] rays)
        {
            ...
            for (int i = 0; i < rays.Length; i++)
            {
                ...
                if (Physics.Raycast(rays[i], out hitInfo, rayRange.value))
                {
        ...
        private RayRepresentation[] GenerateRays(SchemaAgent agent)
        {
            ...
            for (int i = 0; i < rays.Length; i++)
            {
                ...
                Vector3 direction = ((modified + rotatedOffset) - (agent.transform.position + rotatedOffset)) * rayRange.value;
        ...
        public override GUIContent GetConditionalContent()
        {
            ...
            sb.AppendFormat("<color=red>${0}</color> ", String.IsNullOrEmpty(gameObjectKey.name) ? "null" : gameObjectKey.name);
        ...
        private void DrawCone(SchemaAgent agent, bool hit)
        {
            ...
            Handles.DrawLine(tip, agent.transform.position + rotatedOffset + rotation * (topNoRotate + Vector3.up * radius), 2f);
            Handles.DrawLine(tip, agent.transform.position + rotatedOffset + rotation * (topNoRotate + Vector3.down * radius), 2f);
            Handles.DrawLine(tip, agent.transform.position + rotatedOffset + rotation * (topNoRotate + Vector3.left * radius), 2f);
